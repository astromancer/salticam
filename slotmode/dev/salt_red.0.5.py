...
def fixer(t, i, tols=(5e-2,5e-4)):
    ...
    i0 = i; i1 = i+1 #intervals of kct for which to correct
    l0 = (i0*kct+ltol < deltat) & (deltat < i1*kct-utol)
    l = l0 & lb2    

    if ~np.any(l): return None, None#no fix required for this interval

    diff = kct - deltat[l]  #discrepancy in kct
    lrb1 = np.roll(l,-1)
