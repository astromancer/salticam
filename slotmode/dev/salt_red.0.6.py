...
def fixer(t, kct, i, tols=(5e-2,5e-4)):
...
def plot_flagged(ax, l):
    #Mark flagged values
    ax.plot( frames[l], deltat[l], 'o', mfc='none', mec='g',
                mew=1.5, ms=7.5,
                label='flagged' )
    ax.plot( frames[lbad], deltat[lbad], 'o', mfc='none', mec='r',
                ms=10, mew=1.5, 
...
figures = []
i, j = 0, 0
imax = 3                                #maximal interval to apply correction to
while i < imax:
    #For each interval loop until no more points need correcting
    print( '\tj = ', j )
    _tfix, flagged = fixer( tfix, kct, i, tolerances )
    ...
    if _tfix is None:                   #No fix needed for this interval
        ...
        plot_flagged(ax, flagged)
        figures.append( fig )
        tfix = _tfix

fig, ax, _ = show_jitter_stats( tfix )
figures.append( fig )
#plt.show()
